...
- 重做会话同步：现在所有历史会话会统一同步到当前供应商，不再尝试猜测每条会话原来使用的供应商。
- 简化【会话管理】页面，只保留“检查会话”和“同步会话”两个主要操作；取消启动时自动修复，避免软件打开后在后台修改会话。
- 会话列表新增“显示内部会话”开关。默认保持简洁，需要时也可以查看、选择和删除 Codex 自动创建的内部会话。
- 检查结果和提示文案改为面向用户的简短说明；详细信息默认折叠，不再占满会话列表。

### 修复 Bug

- 修复同一个会话文件包含多条记录时被错误判断为供应商冲突、导致无法同步的问题；同步时会一次处理完整会话。
- 修复同步会话后可能出现 `invalid_id_prefix`、`item_*` 被要求使用 `msg*` 格式的问题；聊天消息和工具调用记录不会再被改写。
- 修复同时存在新旧会话数据库时，列表数量重复、显示数量与实际列表不一致的问题；现在会按会话 ID 去重并使用最新记录。
- 修复部分会话看起来“消失”的问题：普通会话和内部会话会被正确区分，内部会话可通过开关随时显示。
- 修复同步期间覆盖仍在写入的聊天记录的风险；正在使用或刚发生变化的会话会自动跳过，退出 Codex 后再次同步即可。
- 修复同步中途失败可能留下部分改动的问题；同步前会创建安全备份，并在失败时尽可能恢复，同时保留期间新增的聊天内容。
- 修复【技能和 MCP】导入弹窗打开后，背景页面仍会跟随滚轮或触控板滑动的问题；弹窗内容仍可独立滚动。
- 修复已经导入的 MCP 每次都会重新出现在【导入已有】列表中的问题；现在只显示尚未纳管的内容。
- 修复【会话管理】页面左上角“会话同步”文字贴边后显示不完整的问题。

### 开发

- 会话同步增加多记录会话、新旧数据库去重、正在使用的会话保护、安全回滚和 `item_*` 内容保持测试；当前 Rust 测试共 76 项通过。
