...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
struct ReleaseUpdateInstallResult {
    version: String,
    asset_name: String,
    download_url: String,
}
...
const RELEASE_UPDATE_PROGRESS_EVENT: &str = "release-update-progress";

fn emit_release_update_progress(
    app: &tauri::AppHandle,
    ...
    let payload = ReleaseUpdateProgress {
        phase: phase.into(),
    ...
    let _ = app.emit(RELEASE_UPDATE_PROGRESS_EVENT, payload);
}

fn normalize_update_proxy_url(value: &str) -> Option<String> {
    ...
    if lower.starts_with("https://") {
        Some(format!("http://{}", &trimmed["https://".len()..]))
    } else {
        Some(trimmed.to_string())
    }
}

fn redact_update_proxy_url(value: &str) -> String {
    let trimmed = value.trim();
...
fn update_network_proxy_from_url(
    label: impl Into<String>,
    value: impl AsRef<str>,
) -> Option<UpdateNetworkProxy> {
    let url = normalize_update_proxy_url(value.as_ref())?;
    if ureq::Proxy::new(&url).is_err() {
        return None;
    }
    Some(UpdateNetworkProxy {
        label: label.into(),
        display_url: redact_update_proxy_url(&url),
        url,
    })
}

fn detect_env_update_proxy() -> Option<UpdateNetworkProxy> {
    const KEYS: &[&str] = &[
        "HTTPS_PROXY",
        "https_proxy",
        "HTTP_PROXY",
    ...
    for key in KEYS {
        if let Ok(value) = env::var(key) {
            if let Some(proxy) = update_network_proxy_from_url(format!("环境变量 {key}"), value)
            {
                return Some(proxy);
            }
        }
...
#[cfg(target_os = "macos")]
fn detect_macos_system_update_proxy() -> Option<UpdateNetworkProxy> {
    ...
    for (label, enable_key, host_key, port_key, scheme) in candidates {
        if values.get(enable_key).map(|value| value == "1") != Some(true) {
            continue;
        }
        let Some(host) = values.get(host_key).map(|value| value.trim()) else {
            continue;
        };
        let Some(port) = values.get(port_key).map(|value| value.trim()) else {
            continue;
        };
        ...
        if let Some(proxy) =
            update_network_proxy_from_url(label, format!("{scheme}://{host}:{port}"))
        {
            return Some(proxy);
        }
    }
...
fn connect_local_proxy_port(port: u16) -> std::io::Result<TcpStream> {
    let address = SocketAddr::from(([127, 0, 0, 1], port));
    let stream = TcpStream::connect_timeout(&address, Duration::from_millis(220))?;
    let _ = stream.set_read_timeout(Some(Duration::from_millis(900)));
    let _ = stream.set_write_timeout(Some(Duration::from_millis(900)));
    Ok(stream)
}

fn is_local_http_proxy_available(port: u16) -> bool {
    let Ok(mut stream) = connect_local_proxy_port(port) else {
        return false;
    };
    ...
    response.starts_with("HTTP/") && (response.contains(" 200 ") || response.contains(" 407 "))
}

fn is_local_socks5_proxy_available(port: u16) -> bool {
    let Ok(mut stream) = connect_local_proxy_port(port) else {
        return false;
    };
...
fn detect_local_update_proxy() -> Option<UpdateNetworkProxy> {
    const HTTP_PORTS: &[u16] = &[7890, 7897, 7899, 8080, 1087, 20171, 6152, 8889];
    const SOCKS5_PORTS: &[u16] = &[7891, 1080, 1086, 10808, 6153];
    for port in HTTP_PORTS {
        if is_local_http_proxy_available(*port) {
            return update_network_proxy_from_url(
                "本地 HTTP 代理",
    ...
    for port in SOCKS5_PORTS {
        if is_local_socks5_proxy_available(*port) {
            return update_network_proxy_from_url(
                "本地 SOCKS5 代理",
...
fn detect_update_network_proxy() -> Option<UpdateNetworkProxy> {
    detect_env_update_proxy()
        .or_else(detect_macos_system_update_proxy)
        .or_else(detect_local_update_proxy)
}

fn build_update_agent(
    timeout: Duration,
    proxy: Option<&UpdateNetworkProxy>,
) -> Result<ureq::Agent> {
    let mut builder = ureq::AgentBuilder::new().timeout(timeout);
    if let Some(proxy) = proxy {
        let configured_proxy = ureq::Proxy::new(&proxy.url).map_err(|e| {
            CodexxError::Config(format!("更新代理配置无效 {}: {e}", proxy.display_url))
        })?;
...
fn compare_release_versions(left: &str, right: &str) -> i32 {
    let left_parts = normalize_release_version(left)
        .split(|c| c == '.' || c == '-')
        .map(|part| part.parse::<i32>().unwrap_or(0))
        .collect::<Vec<_>>();
    let right_parts = normalize_release_version(right)
        .split(|c| c == '.' || c == '-')
        .map(|part| part.parse::<i32>().unwrap_or(0))
        .collect::<Vec<_>>();
    let len = left_parts.len().max(right_parts.len()).max(3);
    for index in 0..len {
        let difference = left_parts.get(index).copied().unwrap_or(0)
...
fn request_latest_release(
    repo: &str,
    proxy: Option<&UpdateNetworkProxy>,
) -> std::result::Result<GithubLatestRelease, String> {
    let url = format!("https://api.github.com/repos/{repo}/releases/latest");
    let agent = build_update_agent(Duration::from_secs(30), proxy).map_err(|e| e.to_string())?;
    let response = agent
        .get(&url)
        .set("Accept", "application/vnd.github+json")
        .set("User-Agent", "Codex-X")
...
fn fetch_latest_release(
    ...
    if let Some(proxy) = proxy {
        return match request_latest_release(repo, Some(proxy)) {
            Ok(release) => Ok(release),
            Err(proxy_error) => {
                emit_release_update_progress(
                    app,
                    "checking",
                    format!(
                        "代理 {} 连接失败，正在直连重试 GitHub Release",
                        proxy.display_url
                    ),
                    None,
                    None,
                );
                request_latest_release(repo, None).map_err(|direct_error| {
                    CodexxError::Config(format!(
                        "读取 GitHub Release 失败: 代理 {} 失败: {}; 直连失败: {}",
                        proxy.display_url, proxy_error, direct_error
                    ))
                })
            }
        };
    }
    request_latest_release(repo, None)
        .map_err(|e| CodexxError::Config(format!("读取 GitHub Release 失败: {e}")))
}

fn release_asset_for_current_platform(assets: &[GithubReleaseAsset]) -> Option<GithubReleaseAsset> {
    let preferred = assets.iter().find(|asset| {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            {
                return name.contains("apple_silicon") || name.contains("aarch64");
            }
            #[cfg(target_arch = "x86_64")]
            {
                return name.contains("intel") || name.contains("x64") || name.contains("x86_64");
            }
    ...
    preferred.cloned().or_else(|| {
        assets
            ...
            .find(|asset| {
                ...
                #[cfg(target_os = "windows")]
                {
                    name.ends_with(".msi") || name.ends_with(".zip")
                }
                #[cfg(all(not(target_os = "macos"), not(target_os = "windows")))]
                {
                    name.ends_with(".deb") || name.ends_with(".rpm") || name.ends_with(".appimage")
                }
            })
            .cloned()
    })
...
fn download_release_asset(
    ...
    let mut active_proxy = proxy;
    let request_download =
        |proxy: Option<&UpdateNetworkProxy>| -> std::result::Result<ureq::Response, String> {
            let agent =
                build_update_agent(Duration::from_secs(300), proxy).map_err(|e| e.to_string())?;
            agent
                .get(url)
                .set("User-Agent", "Codex-X")
                .call()
    ...
    let response = match request_download(proxy) {
        Ok(response) => response,
        Err(proxy_error) if proxy.is_some() => {
            let proxy = proxy.expect("checked is_some");
            active_proxy = None;
            emit_release_update_progress(
                app,
                "downloading",
                format!("代理 {} 下载连接失败，正在直连重试", proxy.display_url),
                Some(0),
                None,
            );
            request_download(None).map_err(|direct_error| {
                CodexxError::Config(format!(
                    "下载更新失败: 代理 {} 失败: {}; 直连失败: {}",
                    proxy.display_url, proxy_error, direct_error
                ))
        ...
        Err(error) => {
            return Err(CodexxError::Config(format!("下载更新失败: {error}")));
        }
    };
    ...
    let download_message = if let Some(proxy) = active_proxy {
        format!("正在通过代理 {} 下载 {asset_name}", proxy.display_url)
    } else {
        format!("正在下载 {asset_name}")
    };
    emit_release_update_progress(
        app,
        "downloading",
    ...
    loop {
        let read = reader
            .read(&mut buffer)
            .map_err(|e| CodexxError::Config(format!("读取更新下载流失败: {e}")))?;
        if read == 0 {
            break;
        }
        file.write_all(&buffer[..read])
            .map_err(|e| CodexxError::Config(format!("写入更新文件失败: {e}")))?;
    ...
    file.flush()
        .map_err(|e| CodexxError::Config(format!("保存更新文件失败: {e}")))?;
    Ok(())
}
...
fn current_macos_app_bundle() -> Result<PathBuf> {
    let exe =
        env::current_exe().map_err(|e| CodexxError::Config(format!("无法定位当前应用: {e}")))?;
...
#[cfg(target_os = "macos")]
fn find_macos_app_bundle(dir: &Path) -> Result<PathBuf> {
...
#[cfg(target_os = "macos")]
fn run_macos_command(command: &str, args: &[&std::ffi::OsStr]) -> Result<String> {
    let output = Command::new(command)
        .args(args)
        .output()
        .map_err(|e| CodexxError::Config(format!("执行 {command} 失败: {e}")))?;
    if output.status.success() {
...
#[cfg(target_os = "macos")]
fn install_macos_dmg_update(app: &tauri::AppHandle, dmg_path: &Path) -> Result<()> {
    let current_app = current_macos_app_bundle()?;
    let app_parent = current_app
        .parent()
        .ok_or_else(|| CodexxError::Config("无法定位当前应用目录".to_string()))?;
    let app_name = current_app
        .file_name()
        .ok_or_else(|| CodexxError::Config("无法读取当前应用名称".to_string()))?;
    let temp_root = env::temp_dir().join(format!(
        "codex-x-update-{}-{}",
        std::process::id(),
        Local::now().timestamp_millis()
    ));
    let mount_dir = temp_root.join("mount");
    fs::create_dir_all(&mount_dir).map_err(|e| io_err(&mount_dir, e))?;
    ...
    emit_release_update_progress(app, "mounting", "正在挂载安装镜像", None, None);
    run_macos_command("hdiutil", &attach_args)?;

    let install_result = (|| {
        let source_app = find_macos_app_bundle(&mount_dir)?;
        let backup_app = app_parent.join(format!(
            "{}.updating-backup-{}",
            app_name.to_string_lossy(),
            Local::now().timestamp_millis()
        ));
        emit_release_update_progress(app, "installing", "正在替换应用程序", None, None);
        fs::rename(&current_app, &backup_app).map_err(|e| io_err(&current_app, e))?;

        let ditto_args = [source_app.as_os_str(), current_app.as_os_str()];
        let copy_result = run_macos_command("ditto", &ditto_args);
        if let Err(copy_error) = copy_result {
            let _ = fs::rename(&backup_app, &current_app);
            return Err(copy_error);
        }

        fs::remove_dir_all(&backup_app).map_err(|e| io_err(&backup_app, e))?;
        Ok(())
    })();

    emit_release_update_progress(app, "cleanup", "正在清理临时文件", None, None);
    let detach_args = [
    ...
    let _ = run_macos_command("hdiutil", &detach_args);
...
#[cfg(not(target_os = "macos"))]
fn install_release_asset_from_file(_app: &tauri::AppHandle, _asset_path: &Path) -> Result<()> {
    Err(CodexxError::Config(
        "当前平台暂不支持从 GitHub Release 自动安装，请打开下载页手动安装。".to_string(),
    ))
}

#[cfg(target_os = "macos")]
fn install_release_asset_from_file(app: &tauri::AppHandle, asset_path: &Path) -> Result<()> {
    install_macos_dmg_update(app, asset_path)
}

fn install_release_update_from_github_inner(
    app: tauri::AppHandle,
    repo: String,
) -> Result<ReleaseUpdateInstallResult> {
    let proxy = detect_update_network_proxy();
    let check_message = if let Some(proxy) = proxy.as_ref() {
        format!(
            "已检测到{} {}，正在通过代理检查 GitHub Release",
            proxy.label, proxy.display_url
        )
    } else {
        "未检测到本地代理，正在直连检查 GitHub Release".to_string()
    };
    emit_release_update_progress(&app, "checking", check_message, None, None);
    let release = fetch_latest_release(&app, &repo, proxy.as_ref())?;
    ...
    if latest_version.trim().is_empty() {
        return Err(CodexxError::Config("GitHub Release 没有版本号".to_string()));
    }
    if compare_release_versions(&latest_version, env!("CARGO_PKG_VERSION")) <= 0 {
        return Err(CodexxError::Config("当前已是最新版本".to_string()));
    }
    let asset = release_asset_for_current_platform(&release.assets).ok_or_else(|| {
        let fallback = release
            .html_url
            .as_deref()
            .unwrap_or("https://github.com/yynxxxxx/Codex-X/releases/latest");
        CodexxError::Config(format!("没有找到当前平台可安装的 Release 资产: {fallback}"))
    })?;
    let temp_dir = env::temp_dir().join(format!(
        "codex-x-release-download-{}-{}",
        std::process::id(),
        Local::now().timestamp_millis()
    ));
    ...
    let result = (|| {
        ...
        install_release_asset_from_file(&app, &asset_path)?;
        emit_release_update_progress(&app, "restarting", "更新已安装，正在重新启动", None, None);
        Ok(ReleaseUpdateInstallResult {
...
#[tauri::command]
async fn install_release_update_from_github(
    app: tauri::AppHandle,
    repo: String,
) -> Result<ReleaseUpdateInstallResult> {
    let task_app = app.clone();
    let result = tauri::async_runtime::spawn_blocking(move || {
        install_release_update_from_github_inner(task_app, repo)
    })
    .await
    .map_err(|e| CodexxError::Config(format!("自动下载并更新失败: {e}")))??;
    app.request_restart();
    Ok(result)
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            list_backups,
            restore_backup,
            install_release_update_from_github,
            open_url,
        ])
