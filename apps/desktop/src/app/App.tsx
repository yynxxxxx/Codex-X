...
import * as officialAccountsApi from "../features/official-accounts/api";
import * as promptsApi from "../features/prompts/api";
import * as providersApi from "../features/providers/api";
import * as sessionsApi from "../features/sessions/api";
import * as skillsMcpApi from "../features/skills-mcp/api";
import * as updatesApi from "../features/updates/api";
import {
  clearStartupWizardSeen,
  getActiveProviderId,
  getAutoRestartCodex,
  getLanguage,
  getPromptInjectionMode,
  getTheme,
  hasSeenStartupWizard,
  markStartupWizardSeen,
  setActiveProviderId as persistActiveProviderId,
  setAutoRestartCodex as persistAutoRestartCodex,
  setLanguage as persistLanguage,
  setPromptInjectionMode as persistPromptInjectionMode,
...
export function App() {
  const [lang, setLang] = React.useState<Lang>(getLanguage);
  const [theme, setTheme] = React.useState<AppTheme>(getTheme);
  const t = dict[lang];
  ...
  const [promptInjectionMode, setPromptInjectionMode] = React.useState<PromptInjectionMode>(getPromptInjectionMode);
  ...
  const [activeProviderId, setActiveProviderId] = React.useState(getActiveProviderId);
  ...
  const [startupWizardOpen, setStartupWizardOpen] = React.useState(() => !hasSeenStartupWizard());
  ...
  React.useEffect(() => {
    persistPromptInjectionMode(promptInjectionMode);
  }, [promptInjectionMode]);

  React.useEffect(() => {
    persistAutoRestartCodex(autoRestartCodex);
  }, [autoRestartCodex]);
  ...
  React.useEffect(() => {
    ...
    void providersApi.buildProviderTomlDraft({
      ...providerForm,
  ...
  const refresh = React.useCallback((includeDiagnostics: boolean) => {
    ...
    if (includeDiagnostics) {
      void codexApi.getStartupDiagnostics(resolvedConfigDir)
        .then((diagnostics) => {
    ...
    void codexApi.getCodexState(resolvedConfigDir)
      .then((next) => {
        ...
        void Promise.allSettled([
          providersApi.listSavedProviders(),
          officialAccountsApi.listOfficialAccounts(next.codexDir),
          promptsApi.listSavedPrompts(),
          promptsApi.getBuiltinPromptStatus(),
  ...
  React.useEffect(() => {
    ...
    void codexApi.getAboutInfo(loadKey)
      .then((about) => {
  ...
  React.useEffect(() => {
    ...
    if (mappedProviderId && mappedProviderId !== activeProviderId) {
      persistActiveProviderId(mappedProviderId);
  ...
  const handleActionResult = (result: ActionResult) => {
    ...
    void Promise.allSettled([
      promptsApi.listSavedPrompts(),
      providersApi.listSavedProviders(),
      officialAccountsApi.listOfficialAccounts(result.state.codexDir),
    ])
  ...
  const restartCodex = React.useCallback(async ({
    ...
    setError("");
    try {
      const result = await codexApi.restartCodexApp();
      if (result.success) {
        setToast(result.message);
      } else if (automatic) {
        setError(lang === "zh"
  ...
  const switchInstructionTemplate = (templateId: string) =>
    call(
      () => promptsApi.enableInstructionTemplate(configDir || null, templateId, promptInjectionMode),
      handleActionResult,
    );

  const disableInstruction = () =>
    call(
      () => promptsApi.disableInstruction(configDir || null),
      handleActionResult,
    );

  const disableExternalInstruction = () =>
    call(
      () => promptsApi.disableExternalInstruction(configDir || null),
      handleActionResult,
    );
  ...
  const openEditBuiltinPrompt = (templateId: string) =>
    call(
      () => promptsApi.getBuiltinPromptDetail(templateId),
      (detail) => {
        setEditingPromptId(null);
  ...
  const savePromptOnly = () => {
    if (editingBuiltinPrompt) {
      call(
        async () => {
          const detail = await promptsApi.saveBuiltinPromptOverride(
            editingBuiltinPrompt.id,
            promptForm.content,
          );
          const statuses = await promptsApi.getBuiltinPromptStatus();
    ...
    call(
      async () => {
        await promptsApi.savePrompt(normalizedPromptForm());
        return promptsApi.listSavedPrompts();
      },
      (promptList) => {
  ...
  const enableSavedPrompt = (id: string) =>
    call(() => promptsApi.enableSavedPrompt(configDir || null, id, promptInjectionMode), handleActionResult);

  const removeSavedPrompt = (id: string) =>
    call(
      async () => {
        await promptsApi.deleteSavedPrompt(id);
        return promptsApi.listSavedPrompts();
      },
      (promptList) => {
        setSavedPrompts(promptList);
  ...
  const importPromptMd = async (file?: File | null) => {
    ...
    try {
      ...
      await promptsApi.savePrompt({
        id: uniqueId(title, savedPrompts.map((item) => item.id)),
        title: filename.replace(/\.md$/i, ""),
        filename,
        content,
      });
      const promptList = await promptsApi.listSavedPrompts();
      setSavedPrompts(promptList);
  ...
  const refreshBuiltinPrompts = async ({ quiet = false }: { quiet?: boolean } = {}) => {
    ...
    try {
      ...
      const request = existingRequest || promptsApi.refreshBuiltinPrompts(configDir || null);
      if (!existingRequest) {
  ...
  const applyProviderConfig = (provider: SavedProvider) => {
    ...
    if (tomlConfig) {
      return providersApi.saveProviderTomlConfig(
        configDir || null,
        tomlConfig,
        provider.apiKey || "",
      );
    }
    return providersApi.switchProvider({
    ...
    return call(
      async () => {
        ...
        if (!providerTomlDirty) {
          ...
          const latestDraft = await providersApi.buildProviderTomlDraft(
            providerForDraft,
            configDir || null,
          );
        ...
        const applied = applyAfterSave
          ? await providersApi.saveActiveProvider(provider, configDir || null)
          : null;
        if (!applyAfterSave) await providersApi.saveProvider(provider);
        const providerList = await providersApi.listSavedProviders();
  ...
  const switchProvider = (provider: SavedProvider) => {
    ...
    return call(
      ...
      (result) => {
        persistActiveProviderId(provider.id);
  ...
  const fetchProviderModels = async () => {
    ...
    try {
      const result = await providersApi.fetchProviderModels(baseUrl, apiKey);
  ...
  const testProvider = async (id: string, baseUrl: string, apiKey?: string | null) => {
    ...
    try {
      const result = await providersApi.testProviderConnection(baseUrl, apiKey || null);
      if (result.ok) {
  ...
  const switchOfficialProvider = () => {
    const before = state;
    return call(
      () => officialAccountsApi.switchOfficialProvider(configDir || null),
      (result) => {
        persistActiveProviderId("");
        setActiveProviderId("");
        handleConfigActionResult(before, result);
  ...
  const switchOfficialAccount = (accountId: string) => {
    const before = state;
    return call(
      () => officialAccountsApi.switchOfficialAccount(configDir || null, accountId),
      (result) => {
        persistActiveProviderId("");
        setActiveProviderId("");
        handleConfigActionResult(before, result);
  ...
  const prepareNewOfficialAccount = () => {
    const before = state;
    return call(
      () => officialAccountsApi.prepareNewOfficialAccount(configDir || null),
      (result) => {
        persistActiveProviderId("");
        setActiveProviderId("");
        setEditingOfficialAccountId(null);
  ...
  const captureCurrentOfficialAccount = async (name: string) => {
    ...
    setError("");
    try {
      const result = await officialAccountsApi.captureCurrentOfficialAccount(configDir || null, name);
      handleActionResult(result);
      return true;
    } catch (captureError) {
  ...
  const deleteOfficialAccount = async (accountId: string) => {
    ...
    setError("");
    try {
      const result = await officialAccountsApi.deleteOfficialAccount(configDir || null, accountId);
      handleConfigActionResult(before, result);
      return true;
    } catch (deleteError) {
  ...
  const restoreOfficialProvider = () =>
    call(
      async () => {
        const result = await officialAccountsApi.restoreOfficialProvider(configDir || null);
        const draft = await officialAccountsApi.getOfficialConfigDraft(configDir || null);
        return { result, draft };
      },
      ({ result, draft }) => {
        if (draft) {
  ...
  const loadCcSwitchOfficial = async () => {
    ...
    setError("");
    try {
      const candidate = await officialAccountsApi.readCcSwitchOfficialAuth();
  ...
  const resetOfficialProvider = () => {
    const before = state;
    return call(
      () => officialAccountsApi.resetOfficialProvider({
        configDir: configDir || null,
  ...
  const importFromCcSwitch = async () => {
    ...
    setError("");
    try {
      const result = await providersApi.importCcSwitchProviders();
      ...
      try {
        const nextState = await codexApi.getCodexState(configDir || null);
  ...
  const openExternalUrl = React.useCallback((url?: string | null) => {
    if (!url) return;
    window.setTimeout(() => {
      void codexApi.openUrl(url).catch(() => {
  ...
  const checkForUpdates = React.useCallback(async ({ quiet = false }: { quiet?: boolean } = {}) => {
    ...
    try {
      ...
      const update = await updatesApi.checkAppUpdate();
  ...
  const loadSkillsMcp = React.useCallback(async ({ quiet = false }: { quiet?: boolean } = {}) => {
    ...
    if (!quiet) setError("");
    try {
      const result = await skillsMcpApi.getSkillsMcpState(request.configDir);
  ...
  const openImportExistingSkillsMcpPreview = async () => {
    ...
    setError("");
    try {
      const preview = await skillsMcpApi.previewExistingSkillsMcp(request.configDir);
      if (!isCurrentSkillsMcpRequest(request.requestId, request.configDirKey)) return;
  ...
  const importExistingSkillsMcp = async () => {
    ...
    setError("");
    try {
      const result = await skillsMcpApi.importExistingSkillsMcp(request.configDir);
      if (!isCurrentSkillsMcpRequest(request.requestId, request.configDirKey)) return;
      skillsMcpLoadedRef.current = request.configDirKey;
  ...
  const checkSkillUpdatesAction = async () => {
    ...
    setError("");
    try {
      const result = await skillsMcpApi.checkSkillUpdates(request.configDir);
      if (!isCurrentSkillsMcpRequest(request.requestId, request.configDirKey)) return;
      skillsMcpLoadedRef.current = request.configDirKey;
      setSkillsMcpState(result);
  ...
  const toggleSkillEnabled = async (id: string, enabled: boolean) => {
    ...
    setError("");
    try {
      const result = await skillsMcpApi.toggleCodexSkill(request.configDir, id, enabled);
      if (!isCurrentSkillsMcpRequest(request.requestId, request.configDirKey)) return;
      skillsMcpLoadedRef.current = request.configDirKey;
      setSkillsMcpState(result);
  ...
  const toggleMcpEnabled = async (id: string, enabled: boolean) => {
    ...
    setError("");
    try {
      const result = await skillsMcpApi.toggleCodexMcp(request.configDir, id, enabled);
      if (!isCurrentSkillsMcpRequest(request.requestId, request.configDirKey)) return;
      skillsMcpLoadedRef.current = request.configDirKey;
      setSkillsMcpState(result);
  ...
  const saveSkillsMcpNote = async (
    itemKind: "skill" | "mcp",
    itemId: string,
    note: string,
  ) => {
    const requestConfigDir = configDir || null;
    const requestConfigDirKey = normalizedConfigDirForComparison(configDir);
    setError("");
    try {
      const result = await skillsMcpApi.updateSkillsMcpNote(
        requestConfigDir,
        itemKind,
        itemId,
        note,
      );
      if (requestConfigDirKey !== activeConfigDirKeyRef.current) {
        throw new Error(lang === "zh" ? "CODEX_HOME 已切换，请重新保存备注" : "CODEX_HOME changed. Save the note again.");
      }
      setSkillsMcpState((current) => {
        if (!current) return current;
        if (result.itemKind === "skill") {
          return {
            ...current,
            skills: current.skills.map((skill) => skill.id === result.itemId
              ? { ...skill, note: result.note ?? null }
              : skill),
          };
        }
        return {
          ...current,
          mcpServers: current.mcpServers.map((server) => server.id === result.itemId
            ? { ...server, note: result.note ?? null }
            : server),
        };
      });
    } catch (noteError) {
  ...
  const installSkillZipFile = async (file?: File | null) => {
    ...
    try {
      ...
      if (!isCurrentSkillsMcpRequest(request.requestId, request.configDirKey)) return;
      const result = await skillsMcpApi.installSkillZip(request.configDir, file.name, bytes);
      if (!isCurrentSkillsMcpRequest(request.requestId, request.configDirKey)) return;
      skillsMcpLoadedRef.current = request.configDirKey;
      setSkillsMcpState(result.state);
      setToast(result.message);
  ...
  const openOfficialEdit = async (accountId?: string) => {
    ...
    try {
      if (accountId) {
        const account = await officialAccountsApi.getOfficialAccount(configDir || null, accountId);
        if (requestId !== officialDraftRequestRef.current) return;
        setOfficialForm({
          name: account.name,
  ...
  const saveOfficialConfig = () => {
    ...
    call(
      () => editingOfficialAccountId
        ? officialAccountsApi.updateOfficialAccount({
          configDir: configDir || null,
        ...
        : officialAccountsApi.saveOfficialConfig({
          configDir: configDir || null,
          model: officialForm.model,
          authJson: officialForm.authJson,
          configText: officialForm.configText,
  ...
  const removeProvider = async (id: string, isCurrent: boolean) => {
    ...
    try {
      ...
      if (isCurrent) {
        switched = await officialAccountsApi.switchOfficialProvider(configDir || null);
        persistActiveProviderId("");
        setActiveProviderId("");
      }
      await providersApi.deleteSavedProvider(id, configDir || null);
      const providerList = await providersApi.listSavedProviders();
      setSavedProviders(providerList);
  ...
  const checkSessions = async () => {
    ...
    setSessionStatus(null);
    await call(
      () => sessionsApi.getSessionSyncStatus(configDir || null),
      (status) => {
  ...
  React.useEffect(() => {
    ...
    void sessionsApi.getSessionSyncStatus(loadKey)
      .then((status) => {
  ...
  const syncSessions = async () => {
    ...
    await call(
      () => sessionsApi.syncSessionsProvider(configDir || null),
      (result) => {
        setSessionStatus(result.status);
        setSelectedSessionIds([]);
        if (result.desktopLifecycleWarning) {
          setToast(result.desktopLifecycleWarning);
        } else if (!result.status.scanComplete) {
  ...
  const deleteSelectedSessions = async () => {
    ...
    setError("");
    try {
      const result = await sessionsApi.deleteCodexSessions(configDir || null, selectedSessionIds);
      setSessionStatus(result.status);
      ...
      } else if (result.desktopLifecycleWarning) {
        setToast(result.desktopLifecycleWarning);
      } else {
        setToast(lang === "zh"
